...
if selected == 'ChatBot':
    ...
    if user_prompt:
        ...
        with st.chat_message("assistant"):
            # Assuming 'gemini_response' is the object returned by the API
            full_text_response = ""

            # Loop through each part of the response
            for part in gemini_response.parts:
                # Check if the part has a 'text' attribute and is not empty
                if hasattr(part, 'text') and part.text:
                    full_text_response += part.text

            # Now, use the full text with st.markdown
